  ...
  implements TypeAdapterFactory
{
      ...
      while (paramClass != Object.class)
      {
        ...
        if (j < i)
        {
          ...
          do
          {
            ...
            localObject = .Gson.Types.resolve(paramTypeToken.getType(), paramClass, localField.getGenericType());
            localObject = createBoundField(paramGson, localField, getFieldName(localField), TypeToken.get((Type)localObject), bool1, bool2);
            localObject = (BoundField)localLinkedHashMap.put(((BoundField)localObject).name, localObject);
          } while (localObject == null);
          throw new IllegalArgumentException(localType + " declares multiple JSON fields named " + ((BoundField)localObject).name);
        }
  ...
  private String getFieldName(Field paramField)
  {
    ...
    for (paramField = this.fieldNamingPolicy.translateName(paramField);; paramField = localSerializedName.value()) {
      return paramField;
    }
  }
...
/* Location:              /prj/BitcoinChecker/tools/dex-tools-2.1-SNAPSHOT/bc-signed-aligned-dex2jar.jar!/com/google/gson/internal/bind/ReflectiveTypeAdapterFactory.class
