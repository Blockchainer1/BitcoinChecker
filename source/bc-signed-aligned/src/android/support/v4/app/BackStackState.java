  ...
  implements Parcelable
{
  ...
  public BackStackState(FragmentManagerImpl paramFragmentManagerImpl, BackStackRecord paramBackStackRecord)
  {
    ...
    if (paramFragmentManagerImpl != null)
    {
      ...
      i = j + 1;
      arrayOfInt[j] = paramFragmentManagerImpl.cmd;
      arrayOfInt = this.mOps;
      int k = i + 1;
      if (paramFragmentManagerImpl.fragment != null) {}
      int m;
      for (j = paramFragmentManagerImpl.fragment.mIndex;; j = -1)
      {
        arrayOfInt[i] = j;
        arrayOfInt = this.mOps;
        j = k + 1;
        ...
        for (j = m + 1; i < k; j++)
        {
          this.mOps[j] = ((Fragment)paramFragmentManagerImpl.removed.get(i)).mIndex;
          i++;
        }
      }
      for (;;)
...
/* Location:              /prj/BitcoinChecker/tools/dex-tools-2.1-SNAPSHOT/bc-signed-aligned-dex2jar.jar!/android/support/v4/app/BackStackState.class
